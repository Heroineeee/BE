# 🐭 끼니콩 (KkiniCong)

> **아동급식카드**를 사용하는 아이들을 위한 편리한 끼니 서비스

---

## 💡 Introduction

![A3포스터1](https://github.com/user-attachments/assets/a56e1195-039c-476e-99ac-53fbb21745b4)
![A3포스터](https://github.com/user-attachments/assets/f9883e1a-a16b-4306-a51b-0a0fcefcd046)

---

## 🧩 Server Architecture

![시스템 아키텍쳐](https://github.com/user-attachments/assets/57855fb9-91e8-4142-930b-aeeeb4033549)

---

## ⚙️ Tech Stack

| 구분        | 사용 기술                                               |
|------------|--------------------------------------------------------|
| **Backend** | Java 17, Spring Boot 3.4.4, Spring Security |
| **Database** | MySQL, AWS RDS, S3, Spring JPA, Query DSL, Redis
| **Infra**   | AWS EC2, Docker, Nginx, Redis    |
| **DevOps**  | GitHub Actions                             |
| **ETC**     | Opensearch, OpenAI API, Kakao Map API, Google Place API, 공공 데이터 포털  |

---

## 📌 ERD

// 수정해서 추가

## 🙋🏻‍♂️ Server Engineers

| Chaeeun | Erika |
|:---------:|:-----:|
| <img src="https://avatars.githubusercontent.com/u/96906242?v=4" width="100" height="100" style="border-radius: 50%;" /><br/><a href="https://github.com/chaen-ing" target="_blank"><img src="https://img.shields.io/badge/chaen--ing-181717?style=for-the-social&logo=github&logoColor=white"/></a> | <img src="https://avatars.githubusercontent.com/u/141746836?v=4" width="100" height="100" style="border-radius: 50%;" /><br/><a href="https://github.com/erika0915" target="_blank"><img src="https://img.shields.io/badge/erika0915-181717?style=for-the-social&logo=github&logoColor=white"/></a> |

